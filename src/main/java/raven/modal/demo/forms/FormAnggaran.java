...
import raven.modal.demo.create.CreateAnggaranForm;
...
@SystemForm(name = "Anggaran", description = "Menu untuk membuat, mengubah, dan menghapus Anggaran pada Akun", tags = {"anggaran"})
public class FormAnggaran extends Form {
    private JTable table;
    ServiceAnggaran service = new ServiceAnggaran();
...
//    }
    ...
    private Component createTable() {
        ...
        // create table model
        Object columns[] = new Object[]{"SELECT", "KODE ANGGARAN", "NAMA AKUN", "JUMLAH ANGGARAN", "PERIODE ANGGARAN"};
...
//        panel.add(title, "gapx 20");
    ...
    private void loadData() {
        try {
            ...
            model.setRowCount(0);
            List<ModelAnggaran> list = service.getAll();
            for (ModelAnggaran d : list) {
    ...
    private Component createHeaderAction() {
        ...
        JButton cmdCreate = new JButton("Tambah Anggaran");
        JButton cmdEdit = new JButton("Ubah Anggaran");
        JButton cmdDelete = new JButton("Hapus Anggaran");
...
//        panel.add(cmdExport);
    ...
    private void showModalAdd() {
        ...
        CreateAnggaranForm create = new CreateAnggaranForm();
        create.loadData(service, null);

        ModalDialog.showModal(this, new SimpleModalBorder(create, "Tambah Anggaran", options,
                (controller, action) -> {
                    if (action == SimpleModalBorder.OK_OPTION) {
                        // save
                        try {
                            service.create(create.getData());
                            Toast.show(this, Toast.Type.SUCCESS, "Anggaran dibuat");
                            loadData();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    private void showModalEdit() {
        ...
        List<ModelAnggaran> list = getSelectedData();
        if (!list.isEmpty()) {
            if (list.size() == 1) {
                ModelAnggaran data = list.get(0);
                System.out.println(data);
                CreateAnggaranForm create = new CreateAnggaranForm();
                create.loadData(service, data);
                ModalDialog.showModal(this, new SimpleModalBorder(create, "Ubah Anggaran [" + data.getNamaAkun() + "] ?", options, (controller, action) -> {
                    if (action == SimpleModalBorder.OK_OPTION) {
                        // edit
                        try {
                            ModelAnggaran dataEdit = create.getData();
                            service.edit(dataEdit);
                            Toast.show(this, Toast.Type.SUCCESS, "Anggaran sudah diperbarui");
                            loadData();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                ...
                Toast.show(this, Toast.Type.WARNING, "Silahkan pilih satu Anggaran saja!");
            }
        } else {
            Toast.show(this, Toast.Type.WARNING, "Silahkan pilih salah satu Anggaran!");
        }
    }
    ...
    private void showModalDelete() {
        ...
        if (!list.isEmpty()) {
            JLabel label = new JLabel("Apakah anda yakin hapus " + list.size() + " Anggaran?");
            label.setBorder(new EmptyBorder(5, 25, 5, 25));
            ModalDialog.showModal(this, new SimpleModalBorder(label , "Konfirmasi Hapus Anggaran?", options, (controller, action) -> {
                if (action == SimpleModalBorder.OK_OPTION) {
                    // delete
                    try {
                        for (ModelAnggaran d : list) {
                            service.delete(d.getKodeAnggaran());
                        }
                        Toast.show(this, Toast.Type.SUCCESS, "Anggaran sudah dihapus!");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    loadData();
            ...
            Toast.show(this, Toast.Type.WARNING, "Silahkan pilih Anggaran yang akan dihapus!");
        }
    }
    private void searchData(String search) {
        try {
            ...
            model.setRowCount(0);
            List<ModelAnggaran> list = service.search(search);
            for (ModelAnggaran d : list) {
                model.addRow(d.toTableRow(table.getRowCount() + 1));
            }
        } catch (Exception e) {
    ...
    private List<ModelAnggaran> getSelectedData() {
        List<ModelAnggaran> list = new ArrayList<>();
        for (int i = 0; i < table.getRowCount(); i++) {
            if ((boolean) table.getValueAt(i, 0)) {
                ModelAnggaran data = (ModelAnggaran) table.getValueAt(i, 1);
                list.add(data);
            }
        }
        return list;
