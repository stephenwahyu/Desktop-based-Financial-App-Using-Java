...
import raven.modal.demo.modelreport.ModelNeracaTable;
...
import raven.modal.demo.service.ServiceNeraca;
...
@SystemForm(name = "Neraca", description = "Menu untuk melihat laporan Neraca", tags = {"neraca"})
public class FormNeraca extends Form {
    private JTable table1;
    private JTable table2;
    ServiceNeraca service = new ServiceNeraca();
    DatePicker datePicker1 = new DatePicker();
    DatePicker datePicker2 = new DatePicker();
    String saldoAset = "", saldoKewajiban = "", saldoEkuitas = "", saldoKewajibanEkuitas = "";
...
//        panel.add(createHeaderAction());
    ...
    private void loadData() {
        try {
            ...
            List<ModelNeraca> list2 = service.getAll(Date.valueOf(LocalDate.now()));
            for (ModelNeraca d : list2) {
    ...
    private void showModalPrint() {
        try {
            List<ModelNeracaTable> list = new ArrayList<>();
            ...
            for (int row = 0; row < model.getRowCount(); row++) {
                ...
                list.add(new ModelNeracaTable(tipeAkun, namaAkun, saldo));
            }

            TableModel model1 = table1.getModel();
            for (int row = 0; row < model1.getRowCount(); row++) {
                String tipeAkun = model1.getValueAt(row, 0).toString();
                String saldo = model1.getValueAt(row, 1).toString();;

                if (tipeAkun == null)
                    continue;
                else if (tipeAkun.equals("Aset"))
                    saldoAset = saldo;
                else if (tipeAkun.equals("Kewajiban"))
                    saldoKewajiban = saldo;
                else if (tipeAkun.equals("Ekuitas"))
                    saldoEkuitas = saldo;
                else if (tipeAkun.equals("Kewajiban + Ekuitas"))
                    saldoKewajibanEkuitas = saldo;
            }
            ...
            ModelNeracaReport dataprint = new ModelNeracaReport(tanggal,saldoAset, saldoKewajiban, saldoEkuitas, saldoKewajibanEkuitas, Demo.DIREKTUR, list);
            JasperPrint print = ReportManager.getInstance().printNeraca(dataprint);
...
//            Toast.show(this, Toast.Type.SUCCESS, "Terprint!");
    ...
    private void showDate(LocalDate search) {
        try {
            ...
            model2.setRowCount(0);
            List<ModelNeracaTotal> list1 = service.getTotal(Date.valueOf(search));
            List<ModelNeraca> list2 = service.getAll(Date.valueOf(search));
            for (ModelNeracaTotal d : list1) {
                if (d.getKategori() == null){
                    continue;
                }
    ...
    private void showDateInterval(LocalDate[] search) {
        try {
            ...
            List<ModelNeracaTotal> list1 = service.getTotalInterval(Date.valueOf(search[0]), Date.valueOf(search[1]));
            List<ModelNeraca> list2 = service.getAllInterval(Date.valueOf(search[0]), Date.valueOf(search[1]));
            for (ModelNeracaTotal d : list1) {
                if (d.getKategori() == null){
                    continue;
                }
    ...
    private Component createTableTotal() {
        ...
        // create table model
        Object columns[] = new Object[]{"TIPE AKUN", "TOTAL SALDO"};
        DefaultTableModel model = new DefaultTableModel(columns, 0) {
...
//        table.getColumnModel().getColumn(0).setHeaderRenderer(new CheckBoxTableHeaderRenderer(table, 0));
    ...
    private void loadDataTotal() {
        try {
            ...
            model1.setRowCount(0);
            List<ModelNeracaTotal> list2 = service.getTotal(Date.valueOf(LocalDate.now()));
            for (ModelNeracaTotal d : list2) {
                if (d.getKategori() == null){
                    continue;
                }
                model1.addRow(d.toTableRow(table1.getRowCount() + 1));
