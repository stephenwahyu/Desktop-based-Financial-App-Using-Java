...
import raven.modal.demo.create.CreatePengeluaranForm;
...
@SystemForm(name = "Transaksi Pengeluaran", description = "Menu untuk membuat, mengubah, dan menghapus Transaksi Pengeluaran", tags = {"transaksi pengeluaran"})
public class FormPengeluaran extends Form {
    private JTable table;
    ServicePengeluaran service = new ServicePengeluaran();
...
//        panel.add(title, "gapx 20");
    ...
    private void showModalPrint() {
        try {
            ...
            JasperPrint print = ReportManager.getInstance().printKeluar(dataprint);
            ...
            try {
                ...
                switch (fileType) {
                    case "pdf":
                        ...
                        pdfExporter.setExporterOutput(new SimpleOutputStreamExporterOutput(selectedFile));
...
//                        JasperExportManager.exportReportToPdfFile(print, selectedFile.getAbsolutePath());
    ...
    private Component createHeaderAction() {
        ...
        JButton cmdCreate = new JButton("Tambah Transaksi Pengeluaran");
        JButton cmdEdit = new JButton("Ubah Transaksi Pengeluaran");
        JButton cmdDelete = new JButton("Hapus Transaksi Pengeluaran");
    ...
    private void showModalAdd() {
        ...
        CreatePengeluaranForm create = new CreatePengeluaranForm();
                ...
                (controller, action) -> {
                    if (action == SimpleModalBorder.OK_OPTION) {
                        ...
                        try {
                            ...
                            Toast.show(this, Toast.Type.SUCCESS, "Transaksi Pengeluaran dibuat");
                            loadData();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    private void showModalEdit() {
        ...
        if (!list.isEmpty()) {
            if (list.size() == 1) {
                ...
                ModalDialog.showModal(this, new SimpleModalBorder(create, "Ubah Transaksi Pengeluaran [" + data.getNamaAkun() + "] ?", options, (controller, action) -> {
                    if (action == SimpleModalBorder.OK_OPTION) {
                        // edit
                        try {
    ...
    private void showModalDelete() {
        ...
        if (!list.isEmpty()) {
            JLabel label = new JLabel("Apakah anda yakin hapus " + list.size() + " Transaksi Pengeluaran?");
            label.setBorder(new EmptyBorder(5, 25, 5, 25));
            ModalDialog.showModal(this, new SimpleModalBorder(label, "Konfirmasi Hapus Transaksi?", options, (controller, action) -> {
                if (action == SimpleModalBorder.OK_OPTION) {
                    // delete
                    try {
                        for (ModelTransaksi d : list) {
                        ...
                        Toast.show(this, Toast.Type.SUCCESS, "Transaksi Pengeluaran sudah dihapus!");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    loadData();
