...
public class ServicePengeluaran {
    public List<ModelTransaksi> getAll() throws SQLException {
        ...
        try {
            ...
            p = con.prepareStatement("select * from transactions join accounts using (account_id) where transaction_type='Pengeluaran'");
            r = p.executeQuery();
    ...
    public List<ModelTransaksi> search(String search) throws SQLException {
        ...
        try {
            con = DatabaseConnection.getInstance().createConnection();
            p = con.prepareStatement("select * from transactions join accounts using (account_id) where transaction_type = 'Pengeluaran' && transaction_id like ? order by transaction_id");
